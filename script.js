...
// AUTOSUGGEST + REDIRECT (WORKS ON ALL PAGES)
window.addEventListener("load", () => {
    ...
    // LOAD ARTICLE IN latestnews.html BASED ON ?id=
    const urlParams = new URLSearchParams(window.location.search);
    const id = urlParams.get("id");
    if (id && window.location.pathname.includes("latestnews.html")) {
        const article = fullArticleContent[id];
        if (!article) return;
...
// Only for latestnews.html – article data
